...
app.use(session({
  secret: functions.config().session?.secret || process.env.SESSION_SECRET || 'your-secret-key',
  resave: false,
  saveUninitialized: false,
  cookie: { 
    secure: false, // Firebase Functions handles HTTPS
    maxAge: 24 * 60 * 60 * 1000 // 24 hours
  }
}));

// Configure multer for file uploads (both email lists and attachments)
const storage = multer.memoryStorage(); // Use memory storage for Firebase Functions
...
const sendEmailViaGmailAPI = async (tokens, userEmail, to, subject, htmlContent, attachments = []) => {
  try {
    ...
    for (const attachment of attachments) {
      ...
      emailContent += 'Content-Transfer-Encoding: base64\r\n\r\n';
      emailContent += attachment.buffer.toString('base64') + '\r\n\r\n';
    }
...
app.get('/privacy', (req, res) => {
  res.redirect('https://bulkmailer-app.web.app/privacy.html');
});

app.get('/terms', (req, res) => {
  res.redirect('https://bulkmailer-app.web.app/terms.html');
});
...
app.get('/auth/google/callback', async (req, res) => {
  try {
    ...
    res.redirect('https://bulkmailer-app.web.app/?login=success');
  } catch (error) {
    console.error('OAuth callback error:', error);
    res.redirect('https://bulkmailer-app.web.app/?login=error');
  }
});
...
app.post('/upload-and-send', requireAuth, upload.any(), async (req, res) => {
  try {
    ...
    if (emailMethod === 'file') {
      ...
      const workbook = XLSX.read(emailListFile.buffer);
